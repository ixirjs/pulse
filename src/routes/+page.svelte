<script lang="ts">
		...
		basicControllers = retrigger(basicControllers, basicBoxes, (el, i) => {
			...
			return animate(
				el,
				{
					...
					backgroundColor: basicOn ? '#a855f7' : '#6366f1'
				},
	...
	const GRAD_A = 'linear-gradient(90deg, #6366f1 0%, #ec4899 100%)';
	const GRAD_B = 'linear-gradient(135deg, #06b6d4 0%, #22c55e 100%)';
	...
	const vtItems = [
		...
		{ id: 'borealis', label: 'Borealis', color: 'bg-rose-500' },
		{ id: 'cosmos', label: 'Cosmos', color: 'bg-emerald-500' },
...
{#snippet trigger(label: string, onclick: () => void, disabled = false)}
	<button
		{onclick}
		{disabled}
		class="rounded-md border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-700 transition-colors hover:bg-zinc-900 hover:text-white disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-zinc-700"
	>
		{label}
	</button>
{/snippet}
...
<main class="mx-auto max-w-6xl px-6 py-16">
	<header class="mb-12 flex items-end justify-between">
		...
		<a
			...
			class="font-mono text-xs text-zinc-400 transition-colors hover:text-zinc-700"
		>
			26 examples
		</a>
	</header>
...
function run() {
  ...
  controllers = boxes.map((el, i) => {
    ...
    return animate(el, {
      ...
      backgroundColor: on ? '#a855f7' : '#6366f1',
    }, { delay: i * 55, duration: 520, easing: backOut });
  });
}`}
		>
    ...
    rotate: { to: 45,  spring: { stiffness: 200, damping: 12 } },
		...
		>
			<div class="grid grid-cols-3 gap-5">
				{#each range(9) as i (i)}
					<div bind:this={springEls[i]} class="size-6 rounded-lg bg-rose-500"></div>
				{/each}
			</div>
...
controllers = els.map((el, i) =>
		...
		>
			<div class="grid grid-cols-4 gap-2">
				{#each range(STAGGER_COUNT) as i (i)}
					<div bind:this={staggerEls[i]} class="size-6 rounded bg-emerald-500"></div>
				{/each}
			</div>
  ...
  .play();`}
		>
			<div bind:this={tlCard} class="w-full max-w-[15rem] rounded-lg border border-zinc-200 p-4">
				<div
					bind:this={tlLabel}
					class="mb-1.5 text-[10px] font-semibold tracking-widest text-violet-500 uppercase"
				>
					New
				</div>
				<div bind:this={tlTitle} class="mb-1 text-sm font-semibold text-zinc-900">Card reveal</div>
				<div bind:this={tlBody} class="mb-3 text-xs text-zinc-500">
					Each child enters on its own offset.
				</div>
				<button
					bind:this={tlCta}
					class="rounded-md bg-violet-600 px-2.5 py-1 text-[11px] font-semibold text-white"
				>
					Action
				</button>
			</div>
...
{/each}`}
			...
			<div class="grid grid-cols-3 gap-2">
				{#each flipItems as item (item)}
					<div
						...
						class="flex size-12 items-center justify-center rounded-lg bg-amber-100 text-xl"
					>
						{item}
					</div>
				{/each}
  ...
  flipFrom(button, btnFrom, { duration: 420, easing: easeInOut });
		...
		>
			<div
				...
				class="overflow-hidden rounded-lg bg-sky-500 text-white"
				class:w-32={!expanded}
				class:w-56={expanded}
				class:p-3={!expanded}
				class:p-5={expanded}
			...
			<button
				...
				class="rounded-md border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-700 transition-colors hover:bg-zinc-900 hover:text-white"
			>
  ...
  spring: { stiffness: 320, damping: 22 },
		...
		>
			<div class="relative h-32 w-full rounded-lg border border-dashed border-zinc-300">
				{#each ['drag', 'me', 'too'] as label, i (label)}
					<div
						...
						style:left={`${30 + i * 28}%`}
						class="absolute top-1/2 -mt-6 -ml-6 flex size-12 cursor-grab items-center justify-center rounded-xl bg-fuchsia-500 text-[11px] font-semibold text-white active:cursor-grabbing"
					>
						{label}
					</div>
				{/each}
			...
			<div class="flex gap-2">
				{#each ['Tap', 'Me', 'Now'] as label, i (label)}
					<button
						...
						class="rounded-lg bg-teal-600 px-4 py-2 text-xs font-semibold text-white"
					>
						{label}
					</button>
				{/each}
			...
			<div class="grid w-full grid-cols-3 gap-2">
				{#each ['Lazy', 'Reveal', 'On', 'Scroll', 'Into', 'View'] as label (label)}
					<div
						...
						class="flex h-14 items-center justify-center rounded-lg bg-orange-100 text-xs font-medium text-orange-800 opacity-0"
					>
						{label}
					</div>
				{/each}
			...
			<div class="flex flex-wrap items-center justify-center">
				{#each chips as chip (chip.id)}
					<button
						...
						class="m-0.75 overflow-hidden rounded-full bg-violet-100 px-3 py-1 text-xs font-medium whitespace-nowrap text-violet-800 hover:bg-violet-200"
					>
						{chip.label} ✕
					</button>
				{/each}
			...
			<div class="flex w-full max-w-60 flex-col">
				{#each rows as row (row.id)}
					<div
						...
						class="my-0.75 flex items-center justify-between rounded-lg bg-pink-100 px-3 py-2 text-xs font-medium text-pink-800"
					>
						{row.label}
						<button
							onclick={() => removeRow(row.id)}
							aria-label="Remove"
							class="text-pink-400 hover:text-pink-700">✕</button
						>
					</div>
				{/each}
			</div>
			...
			<div class="relative h-24 w-full">
				{#each range(PATHS.length) as i (i)}
					<div
						...
						style:top={`${30 + i * 18}%`}
						class="absolute left-2 size-4 rounded bg-cyan-500"
					></div>
				{/each}
			</div>
			...
			<div class="flex items-end gap-6">
				<div class="space-y-1 text-center">
					<svg viewBox="0 0 100 100" class="size-16">
						<path bind:this={morphOpt} d={SQUARE} fill="#6366f1" />
					</svg>
					<div class="font-mono text-[10px] text-zinc-400">optimize: true</div>
				</div>
				<div class="space-y-1 text-center">
					<svg viewBox="0 0 100 100" class="size-16">
						<path bind:this={morphRaw} d={SQUARE} fill="#d4d4d8" />
					</svg>
					<div class="font-mono text-[10px] text-zinc-400">optimize: false</div>
				</div>
			</div>
...
chars.forEach((c, i) =>
  animate(c, { opacity: [0, 1], y: [24, 0], rotate: [-35, 0] },
    { delay: delay(i, chars.length), duration: 440, easing: backOut })
		...
		>
			<div bind:this={splitEl} class="text-2xl font-bold tracking-tight text-zinc-900">
			...
			<div class="flex w-full max-w-60 flex-col gap-1.5">
				{#each reorderList as value (value)}
					<div
						...
						class="flex cursor-grab items-center gap-2 rounded-lg bg-zinc-100 px-3 py-2 text-xs font-medium text-zinc-700 select-none active:cursor-grabbing"
					>
...
animateValue(0, 100, { onUpdate: (v) => (label.textContent = \`\${v}%\`) });`}
		>
			<div bind:this={counterEl} class="text-3xl font-bold tracking-tight text-emerald-600">0</div>
		...
		>
			<div class="relative size-28">
				<svg viewBox="0 0 100 100" class="size-28 -rotate-90">
					<circle cx="50" cy="50" r={RING_R} fill="none" stroke="#e4e4e7" stroke-width="8" />
					<circle
						bind:this={ringEl}
						cx="50"
						cy="50"
				...
				<div
					class="absolute inset-0 grid place-items-center text-2xl font-bold text-zinc-900 tabular-nums"
				>
					{ringPct}%
				</div>
			</div>
		...
		>
			<div class="flex h-32 w-full items-center justify-center">
				<div
					...
					class="flex size-20 touch-none items-center justify-center rounded-2xl bg-fuchsia-500 text-center text-[10px] font-semibold text-white"
				>
					pinch&nbsp;/&nbsp;rotate
				</div>
			</div>
			<p class="text-center text-[11px] text-zinc-400">Use a touch screen or trackpad pinch.</p>
		</DemoCard>
		...
		>
			<div class="flex h-32 w-full items-center justify-center">
				<button
					...
					class="rounded-xl bg-indigo-500 px-6 py-3 text-xs font-semibold text-white"
				>
					Magnetic
				</button>
			</div>
			<p class="text-center text-[11px] text-zinc-400">Move your cursor across the button.</p>
		</DemoCard>
			...
			<div class="flex min-h-32 w-full max-w-60 flex-col gap-1.5">
				{#each swipeItems as value (value)}
					<div
						...
						class="flex cursor-grab items-center justify-between rounded-lg bg-cyan-100 px-3 py-2 text-xs font-medium text-cyan-800 select-none active:cursor-grabbing"
					>
						{value}
			...
			<div class="flex h-32 w-full items-center justify-center gap-2">
				{#each ['Tab', 'through', 'these'] as label (label)}
					<button
						...
						class="rounded-lg bg-emerald-600 px-4 py-2 text-xs font-semibold text-white outline-none focus-visible:ring-2 focus-visible:ring-emerald-300"
					>
						{label}
					</button>
				{/each}
			</div>
			<p class="text-center text-[11px] text-zinc-400">Press Tab to move focus.</p>
		</DemoCard>
		...
		>
			<div class="flex h-32 flex-col items-center justify-center gap-3">
				<button
					...
					class="rounded-xl bg-rose-500 px-6 py-4 text-sm font-semibold text-white select-none"
				>
					Press me
				</button>
				<span class="font-mono text-xs text-zinc-500">{pressStatus}</span>
			</div>
		</DemoCard>
...
<div {@attach wheelable({ scaleBounds: { min: 0.5, max: 3 } })} />`}
		>
			...
			<p class="text-center text-[11px] text-zinc-400">Hover and scroll, or trackpad pinch.</p>
		</DemoCard>
